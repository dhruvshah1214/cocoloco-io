...
public class CLKillstreakHUD : Photon.MonoBehaviour
{
	...
	void UpdateInput()
	{
		...
		ShowScore = (vp_MPMaster.Phase == vp_MPMaster.GamePhase.Playing);
	...
	void DrawTeam(Vector2 position, Vector2 scale, vp_MPTeam team)
	{
		...
		m_Pos.x = position.x;
		m_Size.y = Screen.height - m_Pos.y - 4 * m_Margin;
	...
	void DrawTeams()
	{
		...
		{
			m_Size.x = ((Mathf.Min(Screen.width, 300)) - (m_Margin * 2));
			m_Size.y = 0;
			tSize = m_Size;

			m_Pos = new Vector2 (Screen.width - m_Size.x - m_Margin, m_Margin);
			tPos = m_Pos;
